...
This shows the plate canvas, target-count selector (`1..7`), action buttons, and runtime/benchmark/quality-gate status lines.

![Main panel and runtime status](assets/screenshots/1.png)

### 2) Segmentation Debug View

This highlights left-to-right target selection with green boxes and per-box predicted characters drawn on the debug canvas.

![Segmentation debug view](assets/screenshots/2.png)

### 3) Character Diagnostics and Final Result

This shows per-character diagnostic cards (`Gray ROI → Binary ROI → Normalized → Best Template → Abs Diff`) and the final ground-truth vs prediction summary.
